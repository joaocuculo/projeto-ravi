...
$query_edit_event = "UPDATE events SET title=:title, color=:color, start=:start, end=:end, conteudo=:conteudo, formaPag=:formaPag WHERE id=:id";
...
if ($edit_event->execute()) {
    $retorna = ['status' => true, 'msg' => 'Evento editado com sucesso!', 'id' => $dados['edit_id'], 'title' => $dados['edit_title'], 'color' => $dados['edit_color'], 'start' => $dados['edit_start'], 'end' => $dados['edit_end'], 'conteudo' => $dados['edit_conteudo'], 'formaPag' => $dados['edit_formaPag']];
} else {
